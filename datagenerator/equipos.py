...
async def PlanificacionSchema(server, idx, nombre_planificacion):    
    myobj = await server.nodes.objects.add_object(idx, nombre_planificacion)
    Estado = await myobj.add_variable("ns=2;s=" + nombre_planificacion + ".estado", "estado", 0)
    PA = await myobj.add_variable("ns=2;s=" + nombre_planificacion + ".pa", "pa", 0.0)
    Frecuencia = await myobj.add_variable("ns=2;s=" + nombre_planificacion + ".frecuencia", "frecuencia", 0.0)
    Horas = await myobj.add_variable("ns=2;s=" + nombre_planificacion + ".horas", "horas", 0.0)
